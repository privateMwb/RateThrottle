...
`packaging/recipes/throttlepro/all/conanfile.py`). Scoped to what isn't obvious
...
- **The release tarball extracts to `RateThrottle-<version>`, not
  `ThrottlePro-<version>`** — the GitHub repo is `RateThrottle`; the CMake
  project/library target inside it is `ThrottlePro`. `cd` into whatever
...
```bash
find $HOME/staging/lib/cmake/ThrottlePro -name "*.cmake"
```

Expect `ThrottleProConfig.cmake`, `ThrottleProConfigVersion.cmake`,
`ThrottleProTargets.cmake` (plus a per-config
`ThrottleProTargets-<config>.cmake`) under `lib/cmake/ThrottlePro/`.
...
find_package(ThrottlePro CONFIG REQUIRED)
add_executable(test main.cpp)
target_link_libraries(test PRIVATE ThrottlePro::ThrottlePro)
EOF
...
just constructs a `ThrottlePro::RateLimiter` and calls `allow()` once,
so success already confirms the header parsed, the library linked, and
...
```bash
conan create packaging/recipes/throttlepro/all --version 1.0.0 \
...
`packaging/recipes/throttlepro/all/test_package/` against the freshly-built
package — a `ThrottlePro linked and constructed successfully.` line in
